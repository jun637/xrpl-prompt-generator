...
| `who_customer` | multi | B2C 일반 소비자 / 중소사업자(SME) / 대기업 / 금융기관 / 공공·정부 / 개발자·빌더 / 크리에이터 / B2B2C 파트너 / **★ 거래소·VASP** / **★ 해외 송금 수요자(이주노동자·유학생 등)** |
...
| `why_xrpl` | multi | (라벨: "XRPL 의 어떤 특성을 우리 제품에서 살리고 싶은지") 빠른 결제(3~5초) / 저렴한 수수료 / 크로스보더 결제 표준 / native DEX·토큰 발행 / 규제 친화 이미지 / 글로벌 파트너십 / 한국 커뮤니티 지원 / **★ 성숙한 네트워크(10년+ 운영)** |
...
| `how_stack` | multi | Node.js / Python / Java·Kotlin / React·Next.js / Swift / iOS 네이티브 / Flutter / 자체 백엔드 / AWS / GCP / Azure / **★ 블록체인·Web3 개발 경험** |
...
- 2026-05-22: 외부 피드백 반영. `who_customer` / `who_need` single → multi. `why_xrpl` 라벨을 "왜 XRPL 을 검토 중인지" → "XRPL 의 어떤 특성을 [우리 제품/이 아이디어]에서 살리고 싶은지" 로 변경 (XRPL 장점을 자연스럽게 어필하는 톤). `how_stack` 정리: "클라우드 함수 기반" "Solidity 경험 있음" "Go" "Rust" "iOS·Android 네이티브" 제거, "Swift" 와 "iOS 네이티브" 통합, "블록체인·Web3 개발 경험" 추가. XRPLF 공식 SDK 가 있는 언어 + 모바일 + 인프라 + Web3 경험으로 축약.
